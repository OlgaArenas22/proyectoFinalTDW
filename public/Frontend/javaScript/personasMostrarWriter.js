    ...
    try {
        ...
        data.persons.forEach(personObj => {
            ...
            personaElemento.innerHTML = `
                <a href="personasMostrarWriter.html" onclick="llamarFuncionPersona(${person.id})" data-persona="${person.id}" class="linkIndexPersona"><strong>${person.id}: ${person.name}</strong></a>
                <br>
        ...
        document.querySelectorAll('.linkIndexPersona').forEach(link => {
            link.addEventListener('click', async function(event) {
                ...
                window.location.href = "personasMostrarWriter.html";
            });
        });
    } catch (error) {
